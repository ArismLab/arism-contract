...
library TreeMap {
    ...
    function _isRed(Entry storage _entry) internal view returns (bool) {
    ...
    function _child(
        ...
        uint8 _direction
    ) internal view returns (Entry storage child) {
    ...
    function _makeEntry(
        ...
        uint _key,
        uint _value
    ) internal returns (uint index, Entry storage entry) {
        ...
        _self.entries[index] = Entry(_key, _value, [uint(0), uint(0)], 1, RED);

        entry = _self.entries[index];
    }

    function _rotateSingle(
        Map storage _self,
        Entry storage _root,
        uint _rootIdx,
        uint8 _direction
    ) internal returns (uint newRootIdx) {
        ...
        require(_rootIdx > 0);
        _root.size =
            _leftChild(_self, _root).size +
            _rightChild(_self, _root).size +
            1;
    }

    function _rotateDouble(
        Map storage _self,
        Entry storage _root,
        uint _rootIdx,
        uint8 _direction
    ...
    function _incrementTreeSize(Map storage _self, uint _key) internal {
    ...
    function probe(
        ...
        uint _key,
        uint _value
    ) internal returns (Entry storage entry) {
        ...
        if (_self.rootIdx == 0) {
            ...
            while (true) {
                ...
                if (_isRed(current) && _isRed(previous)) {
                    uint8 parentDirection = grandParent.links[LEFT] == parentIdx
                        ? LEFT
                        : RIGHT;
                    ...
                    if (currentIdx == previous.links[lastDirection]) {
                        ...
                        //
                        //              3,R <-- current
                        grandParent.links[parentDirection] = _rotateSingle(
                            _self,
                            parent,
                            parentIdx,
                            1 - lastDirection
                        ...
                        grandParent.links[parentDirection] = _rotateDouble(
                            _self,
                            parent,
                            parentIdx,
                            1 - lastDirection
    ...
    function put(
        ...
        uint _key,
        uint _value
    ) internal returns (bool replaced, uint oldValue) {
        Entry storage entry = probe(_self, _key, _value);
    ...
    function _applyColorInRemove(
        ...
        _current.color = RED;
        Entry storage newGrandParent = _child(
            _self,
            _grandParent,
            _parentDirection
        );
    ...
    function _decrementTreeSize(Map storage _self, uint _key) internal {
        uint rootIdx = _self.entries[0].links[RIGHT];
        Entry storage current = _self.entries[rootIdx];
        if (rootIdx == 0 || current.size == 0) {
    ...
    function remove(
        Map storage _self,
        uint _key
    ) internal returns (bool removed, uint oldValue) {
        if (_self.rootIdx != 0) {
            // false root head
            uint currentIdx = 0;
            ...
            while (current.links[direction] != 0) {
                ...
                if (
                    !_isRed(current) && !_isChildRed(_self, current, direction)
                ) {
                    if (_isChildRed(_self, current, 1 - direction)) {
                        ...
                        //
                        //                                  2,B
                        parent.links[lastDirection] = _rotateSingle(
                            _self,
                            current,
                            currentIdx,
                            direction
                        ...
                        Entry storage sibling = _self.entries[
                            parent.links[1 - lastDirection]
                        ];
                        ...
                        if (sibling.size > 0) {
                            ...
                            if (
                                !_isChildRed(_self, sibling, LEFT) &&
                                !_isChildRed(_self, sibling, RIGHT)
                            ) {
                                // color flip
                                //       1,R                  1,B
                                //
                                ...
                                uint8 parentDirection = grandParent.links[
                                    LEFT
                                ] == parentIdx
                                    ? LEFT
                                    : RIGHT;
                                ...
                                if (
                                    _isChildRed(_self, sibling, lastDirection)
                                ) {
                                    // double roatation
    ...
    function get(
        Map storage _self,
        uint _key
    ) internal view returns (bool found, uint value) {
    ...
    /// @dev O(1)
    function size(Map storage _self) internal view returns (uint) {
    ...
    function blackHeight(
        Map storage _self
    ) internal view returns (uint height) {
        Entry storage current = _self.entries[_self.rootIdx];
    ...
    function floorEntry(
        Map storage _self,
        uint _key
    ) internal view returns (bool found, uint key, uint value) {
